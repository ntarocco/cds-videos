...
from cds.modules.flows.models import Status as FlowTaskStatus, TaskMetadata
...
def check_transcoding_status():
    ...
    grouped_flow_tasks = _group_tasks_by_opencast_event_id(pending_tasks)

    with session_context as session:
        for flow_tasks in grouped_flow_tasks:
            # the opencast event id is the same for all transcoding tasks
            event_id = flow_tasks[0].payload["opencast_event_id"]
            try:
                (
                ...
                for flow_task in flow_tasks:
                    on_transcoding_failed.apply_async(
                        str(flow_task.id), error_message=error_message
                    )
                current_app.logger.error(
            ...
            master_object_version = as_object_version(
                flow_tasks[0].payload["master_id"]
            )
            for flow_task in flow_tasks:
                for subformat in opencast_published_subformats:
                    if (
                        flow_task.payload["opencast_publication_tag"]
                        in subformat["tags"]
                    ):
                        on_transcoding_completed.apply_async(
                            (
                                ...
                                event_id,
                                flow_task.payload["preset_quality"],
                            ),
                            link_error=on_celery_task_failed.s(
                                data=dict(flow_task_id=str(flow_task.id))
                            ),
                        )
                    elif opencast_workflow_status == "FAILED":
                        on_transcoding_failed.delay(str(flow_task.id))
...
def on_transcoding_completed(
    ...
    flow_task = TaskMetadata.query.get(task_id)

    from cds.modules.deposit.api import deposit_video_resolver

    deposit_id = flow_task.flow.payload["deposit_id"]
    try:
    ...
    ObjectVersionTag.create(obj, "master", master_object_version_version_id)
    ObjectVersionTag.create(obj, "_opencast_event_id", event_id)
    ...
    try:
        download_time, file_size = _write_file_to_eos(url, obj)
...
def on_celery_task_failed(request, exc, traceback, data, **kwargs):
    ...
    set_flow_tasks_to_failed(flow_task_id, repr(exc))


@shared_task
def on_transcoding_failed(flow_task_id, error_message="Transcoding failed"):
    """Update Task status to failed."""
    set_flow_tasks_to_failed(flow_task_id, error_message)


def set_flow_tasks_to_failed(flow_task_id, error_message):
